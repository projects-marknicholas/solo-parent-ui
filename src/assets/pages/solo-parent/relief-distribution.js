...
function ReliefDistribution() {
  ...
  const [data, setData] = useState([
    {
      AccountId: session.soloParentFormData,
      ...
      Status: 'On Process',
      Subject: 'Applying for Relief Distribution',
  ...
  const handleSubmission = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:3001/api/create-user-ticket', {
        method: 'POST',
        headers: {
            ...
            <>
              ...
              {data.map((user, index) => (
                <div key={index} className='right'>
                  <strong>Notes</strong>
                  <p className='result'>{user.Notes || 'Select a user to view notes'}</p>
                  <i>Contact Information: <span><a href={`mailto:${user.ContactEmail}`}>{user.ContactEmail}</a></span></i>
                </div>
              ))}
            </>
          ) : (
